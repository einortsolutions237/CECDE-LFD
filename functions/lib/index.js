...
exports.scheduledReconciliation = exports.reconcileHierarchyCounts = exports.onUserDeleted = exports.processTeamDeletion = exports.handleUserUpdates = exports.processNewUserOnboarding = void 0;
...
// ----------------------------------------------------------------------------
function calculateRank(userData) {
    ...
    if (directs >= 20 && downline >= 1000 && activeDownline >= 100)
        return 'Crown Ambassador';
    if (directs >= 15 && downline >= 500 && activeDownline >= 50)
        return 'Diamond';
    if (directs >= 10 && downline >= 100 && activeDownline >= 25)
        return 'Platinum';
    if (directs >= 8 && downline >= 50 && activeDownline >= 15)
        return 'Gold';
    if (directs >= 5 && downline >= 20 && activeReferrals >= 5)
        return 'Silver';
    if (directs >= 3 && downline >= 5 && activeReferrals >= 2)
        return 'Bronze';
    return 'Member';
}
async function updateRankSafely(userId, currentData, transaction) {
...
// ----------------------------------------------------------------------------
async function validateSponsorHierarchy(newUserId, sponsorId, transaction) {
    var _a;
    if (!sponsorId || sponsorId === 'SYSTEM')
        return null;
    if (newUserId === sponsorId)
        throw new Error("Cannot sponsor oneself.");
...
// ----------------------------------------------------------------------------
async function propagateAncestryUpdatesSafely(sponsorId, totalDiff, activeDiff, dormantDiff, suspendedDiff = 0) {
    var _a;
    let currentId = sponsorId;
    const sponsorPaths = [];
    const maxLevels = 50;
    ...
    for (let i = 0; i < sponsorPaths.length; i += batchSize) {
        ...
        for (const sId of currentChunk) {
            ...
            const updates = {};
            if (totalDiff !== 0)
                updates.totalDownlineCount = admin.firestore.FieldValue.increment(totalDiff);
            if (activeDiff !== 0)
                updates.activeDownlineCount = admin.firestore.FieldValue.increment(activeDiff);
            if (dormantDiff !== 0)
                updates.dormantDownlineCount = admin.firestore.FieldValue.increment(dormantDiff);
            if (suspendedDiff !== 0)
                updates.suspendedDownlineCount = admin.firestore.FieldValue.increment(suspendedDiff);
...
exports.handleUserUpdates = functions.firestore
    ...
    if (activityStateChanged || sponsorChanged || teamChanged) {
        ...
        if (teamChanged) {
            ...
            if (oldData.teamId && oldData.teamId !== 'SYSTEM') {
                ...
                }).catch((e) => { if (e.code !== 5 && e.code !== 'not-found')
                    console.error("Old Team update error", e); });
            }
            // Add to new team
            if (newData.teamId && newData.teamId !== 'SYSTEM') {
                ...
                await db.collection('teams').doc(newData.teamId).update({
                    ...
                    console.error("New Team update error", e); });
            }
        }
        ...
        if (sponsorChanged) {
            if (oldData.sponsorId && oldData.sponsorId !== 'SYSTEM') {
                ...
                await propagateAncestryUpdatesSafely(oldData.sponsorId, -1 - (oldData.totalDownlineCount || 0), // total downline leaves
                (oldState === 'active' ? -1 : 0) - (oldData.activeDownlineCount || 0), (oldState === 'dormant' ? -1 : 0) - (oldData.dormantDownlineCount || 0), (oldState === 'suspended' ? -1 : 0) - (oldData.suspendedDownlineCount || 0));
            }
            if (newData.sponsorId && newData.sponsorId !== 'SYSTEM') {
                ...
                await propagateAncestryUpdatesSafely(newData.sponsorId, 1 + (newData.totalDownlineCount || 0), (nextState === 'active' ? 1 : 0) + (newData.activeDownlineCount || 0), (nextState === 'dormant' ? 1 : 0) + (newData.dormantDownlineCount || 0), (nextState === 'suspended' ? 1 : 0) + (newData.suspendedDownlineCount || 0));
            }
        }
    ...
    if (!activityStateChanged && (newData.directReferralsCount !== oldData.directReferralsCount ||
...
// ----------------------------------------------------------------------------
exports.processTeamDeletion = functions.firestore
    ...
    try {
        ...
        usersQuery.forEach((doc) => userIdsToUpdate.add(doc.id));
        if (oldData.teamLeaderId)
            userIdsToUpdate.add(oldData.teamLeaderId);
        const batchSize = 250;
...
// ----------------------------------------------------------------------------
exports.reconcileHierarchyCounts = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError('permission-denied', 'Must be authenticated');
    }
    const adminDoc = await db.collection('users').doc(context.auth.uid).get();
    if (!adminDoc.exists || (((_a = adminDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== 'admin' && ((_b = adminDoc.data()) === null || _b === void 0 ? void 0 : _b.role) !== 'super_admin')) {
...
exports.scheduledReconciliation = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
    await runReconciliation();
...
async function runReconciliation() {
    ...
    for (const doc of snapshot.docs) {
        ...
        if (userData.sponsorId && userData.sponsorId !== 'SYSTEM') {
            ...
            if (isActive)
                spCounts.activeReferrals++;
            if (isDormant)
                spCounts.dormantReferrals++;
            if (isSuspended)
                spCounts.suspendedReferrals++;
            ...
            while (currentId && currentId !== 'SYSTEM' && maxDepth > 0) {
                ...
                if (isActive)
                    upCounts.activeDownline++;
                if (isDormant)
                    upCounts.dormantDownline++;
                if (isSuspended)
                    upCounts.suspendedDownline++;
    ...
    for (let i = 0; i < teamSnap.docs.length; i += teamBatchSize) {
        ...
        for (const teamDoc of chunk) {
            ...
            const teamUsers = snapshot.docs.filter((d) => d.data().teamId === teamId);
            let totalMembers = 0;
            let activeMembers = 0;
            ...
            const needsUpdate = currentTeamData.totalMembers !== totalMembers ||
...
function initCounts(map, userId) {
    map.set(userId, {
        directReferrals: 0,
        activeReferrals: 0,
        dormantReferrals: 0,
